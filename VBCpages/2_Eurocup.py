...
# eFG% ,TOV% ,OREB% ,DREB% ,Pace ,ORTG ,DRTG ,Net Rating ,AST/TO ,Posesiones
    ...
    df_games_EC = load_encrypted_data(file_path)
...
# P1Rival	P2Rival	Q1Rival	Q2Rival	Q3Rival	Q4Rival	PR1Rival	PR2Rival
    return df_players_EC, df_games_EC

#Cargar los datos
df_players_EC, df_games_EC = load_data()

# Crear una columna con el partido
df_games_EC['Partido'] = df_games_EC.apply(lambda x: f"VBC - {x['Equipo Rival']}"  if x['VBC Local'] else f"{x['Equipo Rival']} - VBC", axis=1)

# Crear una columna con el enlace a la página del partido
df_games_EC['Enlace'] = df_games_EC['ID Partido']

# Crear una columna con la diferencia de puntos
df_games_EC['Diferencia'] = df_games_EC['Puntos VBC'] - df_games_EC['Puntos Rival']

st.title("Estadísticas de la Eurocup")
...
marco = st.selectbox("Selecciona una opción", ["Acumulados de la temporada actual", "Estadísticas jugadores de la temporada", "Líderes de la temporada", "Estadísticas contra un rival", "Estadísticas de un partido", "Líderes históricos", "Récords equipo"])
if marco == "Estadísticas de un partido":
    ...
    season = st.selectbox("Selecciona una temporada", df_games_EC['ID Temporada'].sort_values(ascending=False).unique())
    #Seleccionar un partido
    # Crea una lista para el selectbox con la fecha, el equipo rival y si VBC es el equipo local
    game_list = df_games_EC[df_games_EC['ID Temporada'] == season][['Fecha','Partido']]
    ...
    if game.split(" - ")[1] == "VBC":
        columns_to_show = ['ID Temporada', 'Fase', 'Fecha', 'Hora', 'Puntos VBC','Equipo Rival','Puntos Rival']
    else:
        columns_to_show = ['ID Temporada', 'Fase', 'Fecha', 'Hora', 'Equipo Rival','Puntos Rival', 'Puntos VBC']
    st.dataframe(df_games_EC[(df_games_EC['ID Temporada'] == season) & (df_games_EC['Fecha'] == date)][columns_to_show], hide_index=True)
    ...
    'Tapones Rival', 'TR Rival', 'FPF Rival', 'FPC Rival',
    '+/- Rival', 'Val Rival',
    ...
    if tipo == "Total":
        ...
        max_games = df_players_EC.groupby('Nombre')['ID Partido'].count().sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_games = max_games.rename("Partidos")
        max_points = df_players_EC.groupby('Nombre')['Puntos'].sum().sort_values(ascending=False)
        max_rebounds = df_players_EC.groupby('Nombre')['Rebotes'].sum().sort_values(ascending=False)
        max_assists = df_players_EC.groupby('Nombre')['Asistencias'].sum().sort_values(ascending=False)
        max_steals = df_players_EC.groupby('Nombre')['Robos'].sum().sort_values(ascending=False)
        max_blocks = df_players_EC.groupby('Nombre')['Tapones'].sum().sort_values(ascending=False)
        max_val = df_players_EC.groupby('Nombre')['Val'].sum().sort_values(ascending=False)
        #Cambiar nombre de la columna
        ...
        shots = df_players_EC.groupby('Nombre')[['T1a', 'T1i', 'T2a', 'T2i', 'T3a', 'T3i']].sum()
        ...
        minsh = 30 # Mínimo de tiros para considerar un jugador
        max_t1a = shots[shots['T1a'] >= minsh]['T1a'].sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_t1a = max_t1a.rename("T.Libres")
        max_t2a = shots[shots['T2a'] >= minsh]['T2a'].sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_t2a = max_t2a.rename("T2")
        max_t3a = shots[shots['T3a'] >= minsh]['T3a'].sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_t3a = max_t3a.rename("T3")
        max_t1p = (shots[shots['T1a'] >= minsh]['T1%']).sort_values(ascending=False)
        #Cambiar nombre de la columna
        ...
        max_t2p = (shots[shots['T2a'] >= minsh]['T2%']).sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_t2p = max_t2p.rename("T2%")
        shots['T2'] = shots['T2a']
        max_t2p = max_t2p.to_frame()
        max_t2p['T2'] = shots.loc[max_t2p.index, 'T2']
        max_t3p = (shots[shots['T3a'] >= minsh]['T3%']).sort_values(ascending=False)
        #Cambiar nombre de la columna
        ...
        current_season = df_players_EC['ID Temporada'].max()
        # Selecciona el ID de los jugadores de la temporada actual
        current_players = df_players_EC[df_players_EC['ID Temporada'] == current_season]['ID Jugador'].unique()
        # Filtra los jugadores de la temporada actual
        current_players_df = df_players_EC[df_players_EC['ID Jugador'].isin(current_players)]
        ...
        filtered_players = df_players_EC.groupby('Nombre')['ID Partido'].count()
        filtered_players = filtered_players[filtered_players >= 30].index

        max_games = df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['ID Partido'].count()
        max_points = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Puntos'].sum())
        ...
        max_rebounds = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Rebotes'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_assists = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Asistencias'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_steals = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Robos'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        max_steals['Media'] = round(max_steals['Robos']/max_games,1)
        max_steals = max_steals.sort_values(by='Media',ascending=False).head(lh)
        
        max_blocks = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Tapones'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        max_blocks['Media'] = round(max_blocks['Tapones']/max_games,1)
        max_blocks = max_blocks.sort_values(by='Media',ascending=False).head(lh)
        
        max_val = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['Val'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        # Mostrar los resultados en una tabla
        max_T1a = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['T1a'].sum())
        ...
        max_T2a = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['T2a'].sum())
        # Renombrar la columna
        ...
        max_T3a = pd.DataFrame(df_players_EC[df_players_EC['Nombre'].isin(filtered_players)].groupby('Nombre')['T3a'].sum())
        # Renombrar la columna
    ...
    season = df_games_EC['ID Temporada'].max()
    
    #Calcular total partidos jugados, victorias y derrotas, total como local y como visitante y % de cada uno
    total_games = df_games_EC[df_games_EC['ID Temporada'] == season]['ID Partido'].count()
    total_wins = df_games_EC[df_games_EC['ID Temporada'] == season]['VBC Victoria'].sum()
    total_losses = total_games - total_wins
    total_home = df_games_EC[df_games_EC['ID Temporada'] == season]['VBC Local'].sum()
    total_away = total_games - total_home
    total_home_wins = df_games_EC[(df_games_EC['ID Temporada'] == season) & (df_games_EC['VBC Local'] == 1)]['VBC Victoria'].sum()
    total_away_wins = df_games_EC[(df_games_EC['ID Temporada'] == season) & (df_games_EC['VBC Local'] == 0)]['VBC Victoria'].sum()
    ...
    season_stats_vbc = pd.DataFrame(df_games_EC[df_games_EC['ID Temporada'] == season][['Puntos VBC', 'Rebotes VBC', 'Asistencias VBC', 'Robos VBC', 'Tapones VBC', 'Val VBC']].sum(), columns=['Acumulados'])
    season_stats_vbc['Media'] = round(season_stats_vbc['Acumulados'] / len(df_games_EC[df_games_EC['ID Temporada'] == season]),1)
    
    # Calcular los acumulados de la temporada Rival
    season_stats_rival = pd.DataFrame(df_games_EC[df_games_EC['ID Temporada'] == season][['Puntos Rival', 'Rebotes Rival', 'Asistencias Rival', 'Robos Rival', 'Tapones Rival', 'Val Rival']].sum(), columns=['Acumulados'])
    season_stats_rival['Media'] = round(season_stats_rival['Acumulados'] / len(df_games_EC[df_games_EC['ID Temporada'] == season]),1)
    
    # Datos de tiros VBC
    season_shots_vbc = pd.DataFrame(df_games_EC[df_games_EC['ID Temporada'] == season][['T1a VBC', 'T1i VBC', 'T2a VBC', 'T2i VBC', 'T3a VBC', 'T3i VBC']].sum(), columns=['Acumulados'])
    season_shots_vbc['Media'] = round(season_shots_vbc['Acumulados'] / len(df_games_EC[df_games_EC['ID Temporada'] == season]), 1)
    season_shots_vbc['%'] = round(season_shots_vbc['Acumulados']*100 / season_shots_vbc['Acumulados'].shift(-1), 1)
    
    # Datos de tiros Rival
    season_shots_rival = pd.DataFrame(df_games_EC[df_games_EC['ID Temporada'] == season][['T1a Rival', 'T1i Rival', 'T2a Rival', 'T2i Rival', 'T3a Rival', 'T3i Rival']].sum(), columns=['Acumulados'])
    season_shots_rival['Media'] = round(season_shots_rival['Acumulados'] / len(df_games_EC[df_games_EC['ID Temporada'] == season]), 1)
    ...
    # Mostrar los resultados en una tabla   
    season = df_games_EC['ID Temporada'].max()
    season_games = df_players_EC[df_players_EC['ID Temporada'] == season]
    ...
    mt1a.dataframe(max_t1a, column_config={"T.Libres": st.column_config.TextColumn(width="small")})
    mt1p.dataframe(max_t1p, column_config={"T1%": st.column_config.TextColumn(width="small")})
    mt2a.dataframe(max_t2a, column_config={"T2": st.column_config.TextColumn(width="small")})
    mt2p.dataframe(max_t2p, column_config={"T2%": st.column_config.TextColumn(width="small")})
    mt3a.dataframe(max_t3a, column_config={"T3": st.column_config.TextColumn(width="small")})
    mt3p.dataframe(max_t3p, column_config={"T3%": st.column_config.TextColumn(width="small")})
    ...
    #Puntos partido
    max10_points = df_games_EC.sort_values(by='Puntos VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Puntos VBC', 'Puntos Rival', 'Enlace']
    st.write("Récords en puntos")
    st.dataframe(max10_points[columns_to_show], hide_index=True)
    
    # Diferencia de puntos
    max10_diff = df_games_EC.sort_values(by='Diferencia',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Diferencia', 'Enlace']
    ...
    max10_p1 = df_games_EC.sort_values(by='P1VBC',ascending=False).head(lh)
    max10_p2 = df_games_EC.sort_values(by='P2VBC',ascending=False).head(lh)
    ...
    # Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Puntos', 'Parte', 'Enlace']
    ...
    max10_q1 = df_games_EC.sort_values(by='Q1VBC',ascending=False).head(lh)
    max10_q2 = df_games_EC.sort_values(by='Q2VBC',ascending=False).head(lh)
    max10_q3 = df_games_EC.sort_values(by='Q3VBC',ascending=False).head(lh)
    max10_q4 = df_games_EC.sort_values(by='Q4VBC',ascending=False).head(lh)
    ...
    # Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Puntos', 'Cuarto', 'Enlace']
    st.write("Récords de puntos en un cuarto")
    st.dataframe(max10_quarters[columns_to_show], hide_index=True)
        
    # Rebotes
    max10_rebounds = df_games_EC.sort_values(by='Rebotes VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Rebotes VBC', 'Rebotes Rival', 'Enlace']
    st.write("Récords en rebotes")
    st.dataframe(max10_rebounds[columns_to_show], hide_index=True)
    
    # Asistencias 
    max10_assists = df_games_EC.sort_values(by='Asistencias VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Asistencias VBC', 'Asistencias Rival', 'Enlace']
    st.write("Récords en asistencias")
    st.dataframe(max10_assists[columns_to_show], hide_index=True)

    # Robos
    max10_steals = df_games_EC.sort_values(by='Robos VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Robos VBC', 'Robos Rival', 'Enlace']
    st.write("Récords en robos")
    st.dataframe(max10_steals[columns_to_show], hide_index=True)

    # Tapones
    max10_blocks = df_games_EC.sort_values(by='Tapones VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Tapones VBC', 'Tapones Rival', 'Enlace']
    st.write("Récords en tapones")
    st.dataframe(max10_blocks[columns_to_show], hide_index=True)

    # Valoración
    max10_val = df_games_EC.sort_values(by='Val VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'Val VBC', 'Val Rival', 'Enlace']
    st.write("Récords en valoración")
    st.dataframe(max10_val[columns_to_show], hide_index=True)

    # Tiros de 1,2 y 3 puntos anotados
    # Tiros de 1 punto
    max10_t1a = df_games_EC.sort_values(by='T1a VBC',ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t1a['T1% VBC'] = max10_t1a['T1% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T1a VBC', 'T1% VBC', 'Enlace']
    st.write("Récords en tiros libres")
    st.dataframe(max10_t1a[columns_to_show], hide_index=True)

    # Tiros de 2 puntos
    max10_t2a = df_games_EC.sort_values(by='T2a VBC',ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t2a['T2% VBC'] = max10_t2a['T2% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T2a VBC', 'T2% VBC', 'Enlace']
    st.write("Récords en tiros de 2 puntos")
    st.dataframe(max10_t2a[columns_to_show], hide_index=True)

    # Tiros de 3 puntos
    max10_t3a = df_games_EC.sort_values(by='T3a VBC',ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t3a['T3% VBC'] = max10_t3a['T3% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T3a VBC', 'T3% VBC', 'Enlace']
    st.write("Récords en tiros de 3 puntos")
    st.dataframe(max10_t3a[columns_to_show], hide_index=True)

    # Porcentaje de acierto
    # Tiros de 1 punto
    max10_t1p = df_games_EC.sort_values(by=['T1% VBC','T1a VBC'],ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t1p['T1% VBC'] = max10_t1p['T1% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T1% VBC', 'T1a VBC', 'Enlace']
    st.write("Récords en porcentaje de tiros libres")
    st.dataframe(max10_t1p[columns_to_show], hide_index=True)

    # Tiros de 2 puntos
    max10_t2p = df_games_EC.sort_values(by=['T2% VBC','T2a VBC'],ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t2p['T2% VBC'] = max10_t2p['T2% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T2% VBC', 'T2a VBC', 'Enlace']
    st.write("Récords en porcentaje de tiros de 2 puntos")
    st.dataframe(max10_t2p[columns_to_show], hide_index=True)

    # Tiros de 3 puntos
    max10_t3p = df_games_EC.sort_values(by=['T3% VBC','T3a VBC'],ascending=False).head(lh)
    # Multiplicar por 100 para mostrar el porcentaje
    max10_t3p['T3% VBC'] = max10_t3p['T3% VBC']*100
    #Seleccionar las columnas a mostrar
    columns_to_show = ['ID Temporada', 'Fecha', 'Partido', 'T3% VBC', 'T3a VBC', 'Enlace']
    ...
    season_players = df_players_EC[df_players_EC['ID Temporada'] == season]
    
    # Contar partidos por jugador y puntos totales
    ...
    # Dividir los jugadores en filas de 5
    filas = [jugadores[i:i + 5] for i in range(0, len(jugadores), 5)]
    
    # Mostrar estadísticas para cada fila de jugadores
    for fila in filas:
        cols = st.columns(len(fila))  # Crear columnas dinámicamente según el número de jugadores en la fila
        for col, jugador in zip(cols, fila):
            with col:
                # Mostrar el nombre del jugador
                # Obtener el nombre del jugador usando el ID
                nombre_jugador = df_players_EC[df_players_EC['ID Jugador'] == jugador]['Nombre'].unique()[0]
                st.write(f"**{nombre_jugador}**")
                # Filtrar los datos del jugador
                jugador_stats = season_players[season_players['ID Jugador'] == jugador]
                # Calcular acumulados y medias por partido
                partidos_jugados = len(jugador_stats)
                # Calcular % de victorias
                # Selecciononar id de partidos jugados
                id_partidos_jugados = jugador_stats['ID Partido'].unique()
                # Buscar partidos jugados en el dataframe de partidos y contar victorias
                victorias = len(df_games_EC[df_games_EC['ID Partido'].isin(id_partidos_jugados) & (df_games_EC['VBC Victoria'] == 1)])
                # Calcular porcentaje de victorias
                victorias = round(victorias * 100 / partidos_jugados, 1) if partidos_jugados > 0 else 0.0
                ...
                stats = {
                    'Estadística': ['Partidos', '% Victorias', 'Puntos', 'Rebotes', 'Asistencias', 'Robos', 'Tapones', 'Valoración',
                                   'T. Libres', 'T1%', 'T2 puntos', 'T2%', 'T3 puntos', 'T3%'],
                    'Total': [
                        partidos_jugados,
                        victorias,
                        jugador_stats['Puntos'].sum(),
                        jugador_stats['Rebotes'].sum(),
                        jugador_stats['Asistencias'].sum(),
                        jugador_stats['Robos'].sum(),
                        jugador_stats['Tapones'].sum(),
                        jugador_stats['Val'].sum(),
                        jugador_stats['T1a'].sum(),
                        t1_porcentaje,
                        jugador_stats['T2a'].sum(),
                        t2_porcentaje,
                        jugador_stats['T3a'].sum(),
                        t3_porcentaje
                    ],
                    'Por Partido': [
                        1.0,
                        1.0,
                        round(jugador_stats['Puntos'].mean(), 1),
                        round(jugador_stats['Rebotes'].mean(), 1),
                        round(jugador_stats['Asistencias'].mean(), 1),
                        round(jugador_stats['Robos'].mean(), 1),
                        round(jugador_stats['Tapones'].mean(), 1),
                        round(jugador_stats['Val'].mean(), 1),
                        round(jugador_stats['T1a'].mean(), 1),
                        t1_porcentaje,
                        round(jugador_stats['T2a'].mean(), 1),
                        t2_porcentaje,
                        round(jugador_stats['T3a'].mean(), 1),
                        t3_porcentaje
                    ]
                }
                # Crear un DataFrame con las estadísticas
                jugador_df = pd.DataFrame(stats)
                # Mostrar la tabla
                st.dataframe(jugador_df, hide_index=True, width=300, height=528)
    ...
    equipos = df_games_EC[df_games_EC['ID Temporada'] == season]['Equipo Rival'].unique()  
    # Selecciona el rival
    rival = st.selectbox("Selecciona un rival", equipos)
    # Filtra los partidos contra el rival seleccionado, utiliza el id del equipo
    id_rival = df_games_EC[df_games_EC['Equipo Rival'] == rival]['ID Rival'].unique()[0]
    # Selecciona todos los partidos contra el rival    
    partidos_rival = df_games_EC[df_games_EC['ID Rival'] == id_rival]
    ...
    entrenadores = df_games_EC.groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
    ...
    entrenadores_local = df_games_EC[df_games_EC['VBC Local'] == 1].groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
        Partidos=('ID Partido', 'count'),
        Victorias=('VBC Victoria', lambda x: (x == 1).sum()),
        Derrotas=('VBC Victoria', lambda x: (x == 0).sum())
    ).reset_index()
    ...
    entrenadores_visitante = df_games_EC[df_games_EC['VBC Local'] == 0].groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
        Partidos=('ID Partido', 'count'),
        Victorias=('VBC Victoria', lambda x: (x == 1).sum()),
        Derrotas=('VBC Victoria', lambda x: (x == 0).sum())
    ).reset_index()
