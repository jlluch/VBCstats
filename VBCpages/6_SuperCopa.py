...
def load_data():
    ...
    df_players_SuperCopa = st.session_state.df_players_SuperCopa
    df_games_SuperCopa = st.session_state.df_games_SuperCopa
    return df_players_SuperCopa, df_games_SuperCopa

#Cargar los datos
df_players_SuperCopa, df_games_SuperCopa = load_data()

st.title("Estadísticas de la Supercopa")
...
if marco == "Estadísticas de un partido":
    ...
    season = st.selectbox("Selecciona una temporada", df_games_SuperCopa['ID Temporada'].sort_values(ascending=False).unique())
    #Seleccionar un partido
    # Crea una lista para el selectbox con la fecha, el equipo rival y si VBC es el equipo local
    game_list = df_games_SuperCopa[df_games_SuperCopa['ID Temporada'] == season]['Partido']
    # Invertir el orden de game_list
    game_list = game_list.iloc[::-1]     
    game = st.selectbox("Selecciona un partido", game_list)
    id_game = df_games_SuperCopa[(df_games_SuperCopa['ID Temporada'] == season) & (df_games_SuperCopa['Partido'] == game)].iloc[0]['ID Partido']
    ...
    df_partido = df_games_SuperCopa[(df_games_SuperCopa['ID Partido'] == id_game)][columns_to_show]
    ...
    st.dataframe(df_games_SuperCopa[(df_games_SuperCopa['ID Partido'] == id_game)][columns_to_show], hide_index=True)
    ...
    players_vbc = df_players_SuperCopa[(df_players_SuperCopa['ID Partido'] == id_game)]
    ...
    if tipo == "Total":
        ...
        max_games = df_players_SuperCopa.groupby('Nombre')['ID Partido'].count().sort_values(ascending=False)
        #Cambiar nombre de la columna
        max_games = max_games.rename("Partidos")
        max_points = df_players_SuperCopa.groupby('Nombre')['Puntos'].sum().sort_values(ascending=False)
        max_minutes = df_players_SuperCopa.groupby('Nombre')['Minutos'].sum().sort_values(ascending=False)
        max_rebounds = df_players_SuperCopa.groupby('Nombre')['Rebotes'].sum().sort_values(ascending=False)
        max_assists = df_players_SuperCopa.groupby('Nombre')['Asistencias'].sum().sort_values(ascending=False)
        max_steals = df_players_SuperCopa.groupby('Nombre')['Robos'].sum().sort_values(ascending=False)
        max_blocks = df_players_SuperCopa.groupby('Nombre')['Tapones'].sum().sort_values(ascending=False)
        max_val = df_players_SuperCopa.groupby('Nombre')['Val'].sum().sort_values(ascending=False)
        #Cambiar nombre de la columna
        ...
        shots = df_players_SuperCopa.groupby('Nombre')[['T1a', 'T1i', 'T2a', 'T2i', 'T3a', 'T3i']].sum()
        ...
        current_season = df_players_SuperCopa['ID Temporada'].max()
        # Selecciona el ID de los jugadores de la temporada actual
        current_players = df_players_SuperCopa[df_players_SuperCopa['ID Temporada'] == current_season]['ID Jugador'].unique()
        # Filtra los jugadores de la temporada actual
        current_players_df = df_players_SuperCopa[df_players_SuperCopa['ID Jugador'].isin(current_players)]
        ...
        max_minutes_current = current_players_df.groupby('Nombre')['Minutos'].sum().sort_values(ascending=False).head(1)
    ...
    jornadas_temporada = df_games_SuperCopa[df_games_SuperCopa['ID Temporada'] == season]['Jornada'].unique()
    ...
    season_games = df_games_SuperCopa[df_games_SuperCopa['ID Temporada'] == season]
    ...
    #Seleccionar una temporada, ordenar las temporadas de mayor a menor
    season = st.selectbox("Selecciona una temporada", df_games_SuperCopa['ID Temporada'].sort_values(ascending=False).unique(), index=0)
    #Crear un marco para mostrar los acumulados de la temporada
    ...
    # Calcular los "lh" jugadores con el mayor número de partidos jugados, puntos, rebotes, asistencias, robos, tapones y valoración
    # Mostrar los resultados en una tabla   
    season_games = df_players_SuperCopa[df_players_SuperCopa['ID Temporada'] == season]
    ...
    df_games_records = df_games_SuperCopa.copy()
    ...
    season_players = df_players_SuperCopa[df_players_SuperCopa['ID Temporada'] == season]
    ...
    if resultado_filtro != "Todos":
        ...
        partidos_filtrados = df_games_SuperCopa[(df_games_SuperCopa['ID Temporada'] == season) & 
                                          (df_games_SuperCopa['VBC Victoria'] == victoria_valor)]['ID Partido'].unique()
    ...
    if len(season_players) == 0:
        ...
        for jugador_id in jugadores:
            ...
            nombre_jugador = df_players_SuperCopa[df_players_SuperCopa['ID Jugador'] == jugador_id]['Nombre'].unique()[0]
            ...
            victorias = len(df_games_SuperCopa[df_games_SuperCopa['ID Partido'].isin(id_partidos_jugados) & 
                                         (df_games_SuperCopa['VBC Victoria'] == 1)])
    ...
    # Filtra los equipos
    # Selecciona los equipos únicos de la columna 'ID Rival' del DataFrame df_games_SuperCopa
    # Lee dataframe de equipos de Copa del Rey primera columna IDs del equipo, segunda columna nombre del equipo

    equipos = df_games_SuperCopa['Equipo Rival'].unique()
    ...
    id_rival = df_games_SuperCopa[df_games_SuperCopa['Equipo Rival'] == rival]['ID Rival'].unique()[0]

    # Selecciona todos los partidos contra el rival    
    partidos_rival = df_games_SuperCopa[df_games_SuperCopa['ID Rival']==id_rival]
    ...
    entrenadores = df_games_SuperCopa.groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
    ...
    total_games = len(df_games_SuperCopa)
    total_wins = df_games_SuperCopa['VBC Victoria'].sum()
    total_losses = total_games - total_wins
    total_home = df_games_SuperCopa['VBC Local'].sum()
    total_away = total_games - total_home
    total_home_wins = df_games_SuperCopa[df_games_SuperCopa['VBC Local'] == 1]['VBC Victoria'].sum()
    total_away_wins = df_games_SuperCopa[df_games_SuperCopa['VBC Local'] == 0]['VBC Victoria'].sum()
    total_home_losses = total_home - total_home_wins
    total_away_losses = total_away - total_away_wins
    total_home_wins_percentage = round(total_home_wins * 100 / total_home, 1) if total_home > 0 else 0
    total_away_wins_percentage = round(total_away_wins * 100 / total_away, 1) if total_away > 0 else 0
    ...
    team_stats_vbc = pd.DataFrame(df_games_SuperCopa[['Puntos VBC', 'Rebotes VBC', 'Asistencias VBC', 'Robos VBC', 'Tapones VBC', 'Val VBC']].sum(), columns=['Acumulados'])
    team_stats_vbc['Media'] = round(team_stats_vbc['Acumulados'] / total_games, 1)
    team_stats_rival = pd.DataFrame(df_games_SuperCopa[['Puntos Rival', 'Rebotes Rival', 'Asistencias Rival', 'Robos Rival', 'Tapones Rival', 'Val Rival']].sum(), columns=['Acumulados'])
    team_stats_rival['Media'] = round(team_stats_rival['Acumulados'] / total_games, 1)
    # Datos de tiros VBC
    team_shots_vbc = pd.DataFrame(df_games_SuperCopa[['T1a VBC', 'T1i VBC', 'T2a VBC', 'T2i VBC', 'T3a VBC', 'T3i VBC']].sum(), columns=['Acumulados'])
    ...
    # Datos de tiros Rival
    team_shots_rival = pd.DataFrame(df_games_SuperCopa[['T1a Rival', 'T1i Rival', 'T2a Rival', 'T2i Rival', 'T3a Rival', 'T3i Rival']].sum(), columns=['Acumulados'])
