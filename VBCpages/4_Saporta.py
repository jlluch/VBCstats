...
df_games_Saporta['Partido'] = df_games_Saporta.apply(lambda x: f"VBC - {x['Equipo Rival']}"  if x['VBC Local'] else f"{x['Equipo Rival']} - VBC", axis=1)

# Crear una columna con el enlace a la página del partido
df_games_Saporta['Enlace'] = df_games_Saporta['ID Partido']
...
if marco == "Estadísticas de un partido":
    ...
    game_list = df_games_Saporta[df_games_Saporta['ID Temporada'] == season][['Fecha','Partido']]
    ...
    players_vbc = df_players_Saporta[(df_players_Saporta['ID Temporada'] == season) & (df_players_Saporta['Fecha'] == date)]
