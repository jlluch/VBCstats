...
@st.cache_data 
def load_data():
    df_players_Saporta = st.session_state.df_players_Saporta
    df_games_Saporta = st.session_state.df_games_Saporta
    return df_players_Saporta, df_games_Saporta

#Cargar los datos
df_players_Saporta, df_games_Saporta = load_data()

st.title("Estadísticas de la Copa Saporta")
...
if marco == "Estadísticas de un partido":
    ...
    season = st.selectbox("Selecciona una temporada", df_games_Saporta['ID Temporada'].sort_values(ascending=False).unique())
    #Seleccionar un partido
    # Crea una lista para el selectbox con la fecha, el equipo rival y si VBC es el equipo local
    game_list = df_games_Saporta[df_games_Saporta['ID Temporada'] == season]['Partido']
    ...
    st.dataframe(df_games_Saporta[(df_games_Saporta['ID Partido'] == id_game)][columns_to_show], hide_index=True)
    ...
    players_vbc = df_players_Saporta[(df_players_Saporta['ID Partido'] == id_game)]
    ...
    if tipo == "Total":
        ...
        max_points = df_players_Saporta.groupby('Nombre')['Puntos'].sum().sort_values(ascending=False)
        max_rebounds = df_players_Saporta.groupby('Nombre')['Rebotes'].sum().sort_values(ascending=False)
        max_assists = df_players_Saporta.groupby('Nombre')['Asistencias'].sum().sort_values(ascending=False)
        max_steals = df_players_Saporta.groupby('Nombre')['Robos'].sum().sort_values(ascending=False)
        max_blocks = df_players_Saporta.groupby('Nombre')['Tapones'].sum().sort_values(ascending=False)
        max_val = df_players_Saporta.groupby('Nombre')['Val'].sum().sort_values(ascending=False)
        #Cambiar nombre de la columna
        ...
        shots = df_players_Saporta.groupby('Nombre')[['T1a', 'T1i', 'T2a', 'T2i', 'T3a', 'T3i']].sum()
        ...
        filtered_players = df_players_Saporta.groupby('Nombre')['ID Partido'].count()
        filtered_players = filtered_players[filtered_players >= 30].index

        max_games = df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['ID Partido'].count()
        max_points = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Puntos'].sum())
        ...
        max_rebounds = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Rebotes'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_assists = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Asistencias'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_steals = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Robos'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        max_steals['Media'] = round(max_steals['Robos']/max_games,1)
        max_steals = max_steals.sort_values(by='Media',ascending=False).head(lh)
        
        max_blocks = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Tapones'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        max_blocks['Media'] = round(max_blocks['Tapones']/max_games,1)
        max_blocks = max_blocks.sort_values(by='Media',ascending=False).head(lh)
        
        max_val = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['Val'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        # Mostrar los resultados en una tabla
        max_T1a = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['T1a'].sum())
        ...
        max_T2a = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['T2a'].sum())
        # Renombrar la columna
        ...
        max_T3a = pd.DataFrame(df_players_Saporta[df_players_Saporta['Nombre'].isin(filtered_players)].groupby('Nombre')['T3a'].sum())
        # Renombrar la columna
    ...
    jornadas_temporada = df_games_Saporta[df_games_Saporta['ID Temporada'] == season]['Jornada'].unique()
    ...
    season_games = df_games_Saporta[df_games_Saporta['ID Temporada'] == season]
    ...
    #Puntos partido
    max10_points = df_games_Saporta.sort_values(by='Puntos VBC',ascending=False).head(lh)
    ...
    max10_diff = df_games_Saporta.sort_values(by='Diferencia',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    max10_p1 = df_games_Saporta.sort_values(by='P1VBC',ascending=False).head(lh)
    max10_p2 = df_games_Saporta.sort_values(by='P2VBC',ascending=False).head(lh)
    ...
    max10_q1 = df_games_Saporta.sort_values(by='Q1VBC',ascending=False).head(lh)
    max10_q2 = df_games_Saporta.sort_values(by='Q2VBC',ascending=False).head(lh)
    max10_q3 = df_games_Saporta.sort_values(by='Q3VBC',ascending=False).head(lh)
    max10_q4 = df_games_Saporta.sort_values(by='Q4VBC',ascending=False).head(lh)
    ...
    # Rebotes
    max10_rebounds = df_games_Saporta.sort_values(by='Rebotes VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    # Asistencias 
    max10_assists = df_games_Saporta.sort_values(by='Asistencias VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    # Robos
    max10_steals = df_games_Saporta.sort_values(by='Robos VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    # Tapones
    max10_blocks = df_games_Saporta.sort_values(by='Tapones VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    # Valoración
    max10_val = df_games_Saporta.sort_values(by='Val VBC',ascending=False).head(lh)
    #Seleccionar las columnas a mostrar
    ...
    if resultado_filtro != "Todos":
        ...
        partidos_filtrados = df_games_Saporta[(df_games_Saporta['ID Temporada'] == season) & 
                                          (df_games_Saporta['VBC Victoria'] == victoria_valor)]['ID Partido'].unique()
    ...
    if len(season_players) == 0:
        ...
        for jugador_id in jugadores:
            ...
            nombre_jugador = df_players_Saporta[df_players_Saporta['ID Jugador'] == jugador_id]['Nombre'].unique()[0]
            ...
            victorias = len(df_games_Saporta[df_games_Saporta['ID Partido'].isin(id_partidos_jugados) & 
                                         (df_games_Saporta['VBC Victoria'] == 1)])
    ...
    entrenadores = df_games_Saporta.groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
    ...
    entrenadores_local = df_games_Saporta[df_games_Saporta['VBC Local'] == 1].groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
        Partidos=('ID Partido', 'count'),
        Victorias=('VBC Victoria', lambda x: (x == 1).sum()),
        Derrotas=('VBC Victoria', lambda x: (x == 0).sum())
    ).reset_index()
    ...
    entrenadores_visitante = df_games_Saporta[df_games_Saporta['VBC Local'] == 0].groupby(['ID Entrenador VBC', 'Entrenador VBC']).agg(
        Partidos=('ID Partido', 'count'),
        Victorias=('VBC Victoria', lambda x: (x == 1).sum()),
        Derrotas=('VBC Victoria', lambda x: (x == 0).sum())
    ).reset_index()
