...
def load_data():
    ...
    df_players_CopaRey = st.session_state.df_players_CopaRey
    df_games_CopaRey = st.session_state.df_games_CopaRey
...
if marco == "Estadísticas de un partido":
    ...
    game_list = df_games_CopaRey[df_games_CopaRey['ID Temporada'] == season]['Partido']
    # Invertir el orden de game_list
    game_list = game_list.iloc[::-1]     
    game = st.selectbox("Selecciona un partido", game_list)
    id_game = df_games_CopaRey[(df_games_CopaRey['ID Temporada'] == season) & (df_games_CopaRey['Partido'] == game)].iloc[0]['ID Partido']
    ...
    df_partido = df_games_CopaRey[(df_games_CopaRey['ID Partido'] == id_game)][columns_to_show]
    ...
    st.dataframe(df_games_CopaRey[(df_games_CopaRey['ID Partido'] == id_game)][columns_to_show], hide_index=True)
    ...
    players_vbc = df_players_CopaRey[(df_players_CopaRey['ID Partido'] == id_game)]
    ...
    if tipo == "Total":
        ...
        max_minutes = df_players_CopaRey.groupby('Nombre')['Minutos'].sum().sort_values(ascending=False)
        ...
        max_games = df_players_CopaRey.groupby('Nombre')['ID Partido'].count()
        max_points = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['Puntos'].sum())
        ...
        max_minutes = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['Minutos'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_rebounds = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['Rebotes'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        max_assists = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['Asistencias'].sum())
        # Calcular la media por partido, teniendo en cuenta los partidos que ha jugado cada jugador
        ...
        # Mostrar los resultados en una tabla
        max_T1a = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['T1a'].sum())
        ...
        max_T2a = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['T2a'].sum())
        # Renombrar la columna
        ...
        max_T3a = pd.DataFrame(df_players_CopaRey.groupby('Nombre')['T3a'].sum())
        # Renombrar la columna
    ...
    df_games_records = df_games_CopaRey.copy()
    ...
    total_games = len(df_games_CopaRey)
    total_wins = df_games_CopaRey['VBC Victoria'].sum()
    total_losses = total_games - total_wins
    total_home = df_games_CopaRey['VBC Local'].sum()
    total_away = total_games - total_home
    total_home_wins = df_games_CopaRey[df_games_CopaRey['VBC Local'] == 1]['VBC Victoria'].sum()
    total_away_wins = df_games_CopaRey[df_games_CopaRey['VBC Local'] == 0]['VBC Victoria'].sum()
    total_home_losses = total_home - total_home_wins
    total_away_losses = total_away - total_away_wins
    total_home_wins_percentage = round(total_home_wins * 100 / total_home, 1) if total_home > 0 else 0
    total_away_wins_percentage = round(total_away_wins * 100 / total_away, 1) if total_away > 0 else 0
    ...
    team_stats_vbc = pd.DataFrame(df_games_CopaRey[['Puntos VBC', 'Rebotes VBC', 'Asistencias VBC', 'Robos VBC', 'Tapones VBC', 'Val VBC']].sum(), columns=['Acumulados'])
    team_stats_vbc['Media'] = round(team_stats_vbc['Acumulados'] / total_games, 1)
    team_stats_rival = pd.DataFrame(df_games_CopaRey[['Puntos Rival', 'Rebotes Rival', 'Asistencias Rival', 'Robos Rival', 'Tapones Rival', 'Val Rival']].sum(), columns=['Acumulados'])
    team_stats_rival['Media'] = round(team_stats_rival['Acumulados'] / total_games, 1)
    # Datos de tiros VBC
    team_shots_vbc = pd.DataFrame(df_games_CopaRey[['T1a VBC', 'T1i VBC', 'T2a VBC', 'T2i VBC', 'T3a VBC', 'T3i VBC']].sum(), columns=['Acumulados'])
    ...
    # Datos de tiros Rival
    team_shots_rival = pd.DataFrame(df_games_CopaRey[['T1a Rival', 'T1i Rival', 'T2a Rival', 'T2i Rival', 'T3a Rival', 'T3i Rival']].sum(), columns=['Acumulados'])
